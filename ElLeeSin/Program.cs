 ...
 namespace ElLeeSin
{
    ...
    internal static class Program
    {
        ...
        private static void CastE()
        {
            ...
            if (EState)
            {
                var target = HeroManager.Enemies.Where(x => x.IsValidTarget(spells[Spells.E].Range)).ToList();
            ...
            else
            {
                ...
                if ((PassiveStacks == 0 || target.Count >= 2)
                    || target.Any(t => t.Distance(Player) > Orbwalking.GetRealAutoAttackRange(Player)))
                {
                    spells[Spells.E].Cast();
                }
            }
        ...
        private static void CastQ(Obj_AI_Base target, bool smiteQ = false)
        {
            ...
            if (prediction.Hitchance >= HitChance.Low && prediction.CastPosition.Distance(ObjectManager.Player.ServerPosition) < spells[Spells.Q].Range)
            {
                spells[Spells.Q].Cast(target);
            }
            else if (ParamBool("ElLeeSin.Smite.Q") &&  target.IsValidTarget(spells[Spells.Q].Range)
        ...
        private static void Combo()
        {
                ...
                && (!ParamBool("ElLeeSin.Combo.W") || !ParamBool("ElLeeSin.Combo.Mode.WW")))
            {
                ...
                if (WState && target.IsValidTarget(ObjectManager.Player.AttackRange + 50))
                {
        ...
        private static void JungleClear()
        {
            try
            {
                ...
                if (spells[Spells.W].IsReady() && ParamBool("ElLeeSin.Jungle.W"))
                {
                    ...
                    if (WState && minion.IsValidTarget(ObjectManager.Player.AttackRange + 50))
                    {
                        spells[Spells.W].Cast();
                        LastW = Environment.TickCount;
                    }
