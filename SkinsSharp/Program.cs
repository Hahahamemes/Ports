 ...
 namespace SkinsSharp
{
    class Program
    {
        ...
        {
            EloBuddy.SDK.Events.Loading.OnLoadingComplete += GameLoad;
        }
        ...
        static void FloatPropertyChange(GameObject sender, GameObjectFloatPropertyChangeEventArgs args)
        {
            try
            {
                if (!(sender is AIHeroClient) || args.Property != "mHP" || sender.Name != ObjectManager.Player.Name && !menu.Item("forall").GetValue<bool>())
                {
                    return;
                }
